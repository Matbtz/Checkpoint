...
export async function updateLibraryEntry(
  ...
  if (data.customCoverImage && typeof data.customCoverImage === 'string') {
    ...
    console.log(`[updateLibraryEntry] Extracting colors for: ${data.customCoverImage.substring(0, 50)}...`);

    try {
      const colors = await extractDominantColors(data.customCoverImage);
      console.log(`[updateLibraryEntry] Extraction result:`, colors);
      ...
      console.error("[updateLibraryEntry] Failed to extract colors for custom cover:", e);
