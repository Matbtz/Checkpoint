...
export function GameCard({ item, paceFactor = 1.0, onClick, primaryColor, secondaryColor }: GameCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Layer 2: Content Grid - Optimized for center width */}
      <div className="relative z-20 grid h-full grid-cols-[70px_1fr_54px] sm:grid-cols-[90px_1fr_64px] gap-3 p-3 sm:p-4">
        ...
        {/* Column 2: Main Content (Maximum Space for Title) */}
        ...
        {/* Column 3: Score (Pushed Flush to Right) */}
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 grid h-full grid-cols-[80px_1fr_60px] sm:grid-cols-[100px_1fr_80px] gap-4 p-4">
        ...
        <div className="flex flex-col justify-between min-w-0 py-0.5">
            <div>
                <h2 className="text-xl sm:text-2xl font-black uppercase leading-tight text-white line-clamp-2 tracking-tight">
                    {game.title}
                </h2>

                <div className="flex items-center gap-2 font-inter text-xs font-extralight text-zinc-400 mt-0.5">
